...
def _parse_hle_judge_content(raw_content: str) -> HLEExtractedAnswer:
    """Parse HLE judge output from structured or free-form model text."""
    import json
    ...
    try:
        ...
        return HLEExtractedAnswer.model_validate(payload)
    except Exception:
        pass

    correct_match = re.search(r"correct['\"]?\s*[:=]\s*['\"]?(yes|no)", text, re.I)
    if not correct_match:
        raise ValueError(f"Could not parse HLE judge response: {raw_content[:500]}")

    confidence_match = re.search(r"confidence['\"]?\s*[:=]\s*(\d+)", text, re.I)
    reasoning_match = re.search(
        r"reasoning['\"]?\s*[:=]\s*['\"]?(.+?)(?:['\"]?\s*,\s*correct|$)",
        text,
        re.I | re.S,
    )
    extracted_match = re.search(
        r"extracted_final_answer['\"]?\s*[:=]\s*['\"]?(.+?)['\"]?\s*,",
        text,
        re.I | re.S,
    )
    return HLEExtractedAnswer(
        extracted_final_answer=(extracted_match.group(1).strip() if extracted_match else ""),
...
async def verify_answer_llm_hle(
    ...
    model = os.getenv("EVAL_LLM_MODEL_NAME", "o3-mini-2025-01-31")

    if model.startswith("o3") or model.startswith("gpt-4o"):
        response = await openai_client.beta.chat.completions.parse(
            model=model,
            max_completion_tokens=4096,
            messages=[{"role": "user", "content": prompt}],
        ...
        content = _parse_hle_judge_content(raw)
...
async def verify_answer_llm_finsearchcomp(
    ...
    try:
        # NOTE: official judge is gpt-4o-mini; overridable via EVAL_LLM_MODEL_NAME
        response = await openai_client.chat.completions.create(
            model=os.getenv("EVAL_LLM_MODEL_NAME", "gpt-4o-mini"),
            messages=messages,

...
**对照格 w/o self-evolve** = Memory 开 + 进化前 Skill `0a931278001c`（无 Hermes 自进化）。
...
| **on** | memory-only（own_glm4） | **w/o self-evolve**（own_glm3） | **FINAL** full R1（own_glm3） |

### 已完成：w/o self-evolve（24 个月全窗）

Run：`ablation/runs/mem_ablation_24m/arms/baseline`（24/24 月，memory ON，Skill=`0a931278001c`）。
...
全窗附注：月胜率 54.2%，费用 ¥37,611，无效月 0。

**对照口径**：上表是「有 Memory、无自进化」的底座；待 FINAL / skill-only / memory-only / plain 跑完后，同一快照、同一回放器写入下表，并与 w/o self-evolve 做配对对比。

### 其余格子（进行中，跑完后回填）

| 格子 | 含义 | key | 状态 |
|------|------|-----|------|
| FINAL (full R1) | Memory + `3aebb813bd33` | own_glm3 | 排队/进行中 |
| skill-only R1 | 仅 `3aebb813bd33`，无 Memory | own_glm5 | 进行中 |
| memory-only | 仅 Memory，无 Skill | own_glm4 | 进行中 |
...
| `ablation/` | Leave-one-out 消融（FINAL=`3aebb813bd33`；含 w/o self-evolve） |
...
*最后更新：2026-07-16（消融表 + w/o self-evolve 24m 指标；其余格进行中）*

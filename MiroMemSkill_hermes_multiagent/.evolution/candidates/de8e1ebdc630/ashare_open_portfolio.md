...
2. **从不同来源建立候选集**：委托筛选员子代理（`agent-screener` 的 `execute_subtask`），
   让它分别寻找成长质量、合理估值、高流动性、低波动防御和非极端价格状态的股票；
   子任务必须写明 `as_of`、各路口径与数量要求。每一路只提供候选，不直接决定持仓。
3. **做批量交叉验证**：去重后将不超过 20 只候选连同 `as_of` 委托深研员子代理
   （`agent-analyst` 的 `execute_subtask`）做批量成长质量对比；必要时让它进一步查
   个股财务、估值和价格历史，并给出逐股结论与剔除建议。

...
class Orchestrator:
    ...
    async def run_sub_agent(
        ...
        while turn_count < max_turns:
            ...
            for call in tool_calls[0][:max_tool_calls]:
                ...
                try:
                    # Same pool-escape / lookahead guard as the main-agent loop;
                    # sub-agents make the per-stock calls in the multi-agent setup.
                    _validate_ashare_trader_tool_call(
                        tool_name,
                        arguments,
                        getattr(self, "_active_task_metadata", None),
                    )
    ...
    async def run_main_agent(
        ...
        # Stash task metadata so run_sub_agent can enforce the same
        # ashare-trader guards (pool escape, lookahead as_of) on sub-agent calls.
        self._active_task_metadata = metadata
            ...
            try:
                ...
                if memory_store:
                    memory_block = build_memory_context_block(
                        ...
                        skill_preview_max_chars=int(
                            self.cfg.memory.get("skill_preview_max_chars", 1500)
                        ),

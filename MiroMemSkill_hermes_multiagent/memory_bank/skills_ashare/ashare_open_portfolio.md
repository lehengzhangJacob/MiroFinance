...
dependencies: [ashare_universe_stats, ashare_market_breadth, ashare_index_history, agent-screener, agent-analyst]
triggers:
  - 全A股
  - 开放池
  - 组合仓位
...
2. **从不同来源建立候选集**：委托筛选员子代理（`agent-screener` 的 `execute_subtask`），
   **一次委托**覆盖全部筛选路（成长质量、合理估值、高流动性、低波动防御、非极端价格
   状态），在 subtask 中把各路口径、窗口与数量要求全部列明，并写明 `as_of`；
   仅当报告明显缺路时才补一次委托。每一路只提供候选，不直接决定持仓。
3. **做批量交叉验证**：去重后将不超过 20 只候选连同 `as_of` **一次委托**深研员子代理
   （`agent-analyst` 的 `execute_subtask`）做批量成长质量对比、财务与估值验证，
   要求它给出逐股结论与剔除建议。
...
7. **最终复核**：检查代码可交易、权重和为 1、费用可承受、没有使用 `as_of` 之后的信息。
   子代理报告中的关键结论如互相矛盾，以更保守的一方为准或再委托一次核实。

...
class SyntheticDatasetBuilder:
    ...
    def generate(
        ...
        lm = make_dspy_lm(self.config.judge_model)
        cases_raw = None
        last_error = None

        for attempt in range(3):
            batch = n if attempt == 0 else max(5, n // 2)
            ...
            try:
                cases_raw = self._parse_test_cases_json(result.test_cases)
                break
        ...
        if cases_raw is None:
            raise ValueError(
                f"Could not parse test cases from LLM output after retries: {last_error}"
            )
    ...
    @staticmethod
    def _parse_test_cases_json(raw: str) -> list:
        try:

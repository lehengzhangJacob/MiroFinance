...
def _reflective_skill_rewrite(
    skill_body: str,
    rubric_hint: str,
    reflection_lm,
) -> str:
    """One-shot skill rewrite when GEPA leaves instructions unchanged."""

    class RewriteSkill(dspy.Signature):
        """Rewrite agent skill instructions to follow Conventional Commits properly.
        Required: type(scope): subject header, types feat|fix|docs|chore|refactor|test|ci|style,
        subject <= 72 chars, no trailing period, body uses - dash bullets, no ALL CAPS subjects, no emoji.
        Output ONLY the markdown skill body (no YAML frontmatter)."""
        current_skill: str = dspy.InputField()
        rubric_hint: str = dspy.InputField()
        improved_skill: str = dspy.OutputField()

    rewriter = dspy.ChainOfThought(RewriteSkill)
    with dspy.context(lm=reflection_lm):
        result = rewriter(current_skill=skill_body, rubric_hint=rubric_hint)
    return str(result.improved_skill).strip()
...
def evolve(
    ...
    trace_path = Path("output") / skill_name / trace_ts / "gepa_trace.jsonl"
    ...
    try:
        ...
        try:
            ...
            console.print("[red]✗ MIPROv2 fallback requires optuna: pip install optuna[/red]")
            raise
    ...
    # ── 6. Extract evolved skill text from signature instructions ───────
    evolved_body = extract_skill_instructions(optimized_module)
    if evolved_body.strip() == skill["body"].strip():
        console.print("[yellow]GEPA did not change skill instructions — trying reflective rewrite[/yellow]")
        rubric_hint = (
            "Eval rubric requires Conventional Commit type(scope): subject, "
            "subject <= 72 chars, no period at end, dash body bullets, no ALL CAPS, no emoji."
        )
        evolved_body = _reflective_skill_rewrite(skill["body"], rubric_hint, reflection_lm)
        optimized_module = SkillModule(evolved_body)
    ...
    # Deploy to hermes-agent stub when skill text actually changed
    if evolved_body.strip() != skill["body"].strip():
        skill_path.write_text(evolved_full, encoding="utf-8")
        console.print(f"\n  [green]Deployed evolved skill → {skill_path}[/green]")
